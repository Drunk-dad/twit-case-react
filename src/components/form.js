...
export const Form = () => {
  ...
  const currentLength = inputValue.length + fileList.length * 5;
  ...
  return (
    <form className="form_twitt" onSubmit={submitHandler}>
      <InputText
        ...
        isError={currentLength > MAX_VALUE}
      />
      <Counter maxValue={MAX_VALUE} counterValue={currentLength} />
      <div className="input_file_wrp">
        <InputFile onChange={addFile} />
        <FileList files={fileList} onDelete={deleteFileHandler} />
        <Button isDisabled={currentLength > MAX_VALUE || currentLength < 1} />
      </div>
    </form>
  );
};
